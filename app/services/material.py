...
def search_real_images_web(
    search_term: str,
    limit: int = 5,
) -> List[MaterialInfo]:
    items = []
    # 1. Query DuckDuckGo Real Web Images API
    try:
        url = f"https://duckduckgo.com/?q={quote_plus(search_term)}"
        req_headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"}
        r_page = requests.get(url, headers=req_headers, timeout=6)
        if r_page.status_code == 200:
            m = re.search(r'vqd=([\d-]+)', r_page.text) or re.search(r'vqd="([\d-]+)"', r_page.text)
            if m:
                vqd = m.group(1)
                img_api = f"https://duckduckgo.com/i.js?q={quote_plus(search_term)}&vqd={vqd}&o=json"
                r_img = requests.get(img_api, headers=req_headers, timeout=6)
                if r_img.status_code == 200:
                    ...
                    for res in results[:limit]:
                        ...
                        if img_url and img_url.startswith("http"):
                            item = MaterialInfo()
                            item.provider = "web_real_photo"
                            item.url = img_url
                            item.duration = 4.0
                            items.append(item)
    except Exception as e:
        logger.debug(f"[Material] DDG real image search error for '{search_term}': {e}")

    # 2. Query Stealth Scraper Google Image Search API if DDG returned zero items
    if not items:
        try:
            scraper_url = f"http://127.0.0.1:4050/api/search_images?q={quote_plus(search_term)}&limit={limit}"
            r_scrape = requests.get(scraper_url, timeout=10)
            if r_scrape.status_code == 200:
                img_urls = r_scrape.json().get("images", [])
                for u in img_urls:
                    item = MaterialInfo()
                    item.provider = "google_real_photo"
                    item.url = u
                    item.duration = 4.0
                    items.append(item)
        except Exception as e:
            logger.debug(f"[Material] Stealth Scraper Google image search error for '{search_term}': {e}")
...
def search_real_web_photos(query: str, limit: int = 5) -> List[str]:
    """
    Fetches real high-resolution web photo URLs for a given topic using Bing Images engine.
    """
    import urllib.parse
    ...
    try:
        with urllib.request.urlopen(req, timeout=8) as resp:
            ...
            for u in murls:
                u = u.replace("\\/", "/")
                if u.startswith("http") and not any(b in u.lower() for b in [
                    "logo", "avatar", "icon", "banner", "button", "ytimg", "youtube", "compilation", "thumbnail", "listicle", "collage",
                    "alamy", "c8.alamy", "gettyimages", "shutterstock", "istockphoto", "dreamstime", "depositphotos", "123rf", "stockphoto"
                ]):
                    if u not in clean_urls:
                        clean_urls.append(u)
            return clean_urls[:limit]
    except Exception as e:
        logger.debug(f"[Material] Bing real photo search exception for '{query}': {e}")
    return []
